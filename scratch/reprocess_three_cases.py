...
target_rcepts = ['20260520000202', '20260521000651', '20260521000613']
rows = []
for r_no in target_rcepts:
    r = conn.execute("SELECT rcept_no, report_nm, corp_code, rcept_dt, raw_text FROM filings WHERE rcept_no = ?", (r_no,)).fetchone()
    if r:
        rows.append(r)
...
for rcept_no, report_nm, corp_code, rcept_dt, raw_text in rows:
    ...
    if not corp_name:
        if rcept_no == '20260520000202':
            corp_name = "삼성전자"
        elif rcept_no == '20260521000651':
            corp_name = "하나금융지주"
        elif rcept_no == '20260521000613':
            corp_name = "미래에셋증권"

    # 기존 DB 레코드 삭제 (중복 방지 및 재생성 강제)
    ...
    print(f"[{rcept_no}] {corp_name} - {report_nm} Reprocessed & Saved successfully!")
