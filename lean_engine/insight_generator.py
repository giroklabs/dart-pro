...
class InsightGenerator:
    ...
    def generate_daily_reports(self, target_date: str = None):
        ...
        for i, row in enumerate(targets):
            ...
            작성 지침:
            1. 공시 원문의 법적 문구, 주석(예: 주1, 주2), 불필요하게 긴 텍스트를 절대로 그대로 복사하지 마세요.
            2. 취득 주식 수, 계약 금액, 증자 규모 등 핵심 숫자는 간결하고 깔끔하게 요약(예: "이번에 X주를 추가 취득하여...")하고, 기업 규모 대비 어느 정도 비중인지 그 의미를 쉽게 풀어 설명하세요.
            3. 이 공시가 기업의 실적 및 주가 흐름에 미칠 단기/장기적 영향(호재·악재 여부)을 객관적으로 분석하세요.
            4. 개인 투자자가 앞으로 주목해야 할 핵심 지표와 어떻게 대응해야 할지에 대한 명확한 관전 포인트(액션 가이드)를 제시하세요.
            ...
            {{
                ...
                "summary": "원문의 주석이나 법적 문구를 제외하고, 변경된 핵심 수치와 전체 분석 내용의 핵심만을 일반인이 이해하기 쉽게 3문장 이내로 깔끔하게 요약 (줄바꿈 없이)",
