...
class SummaryRuleEngine:
    ...
    def score_sentence(self, sentence: str, sent_order: int, total_sents: int) -> float:
        ...
        if any(k in norm_sent for k in ['최종배정금액', '모집총액', '총발행금액', '사채의종류', '기초자산종목명', '기초자산명', '발행조건확정', '수요예측결과', '확정금액']):
            score += 20.0
        ...
        if any(k in norm_sent for k in ['70%에미달', '70%에해당', '새로운전환가액', '발행당시전환가액', '안분배정', '선배정', '발행취소', '초과청약', '경합하는경우']):
            score -= 20.0
