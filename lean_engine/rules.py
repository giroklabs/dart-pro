...
class SummaryRuleEngine:
    ...
    def score_sentence(self, sentence: str, sent_order: int, total_sents: int) -> float:
        ...
        # 채무증권/지수증권 등의 최종 배정 및 모집총액 우선순위 가중치 격상
        if any(k in sentence for k in ['최종 배정 금액', '최종 배정금액', '모집 총액', '모집총액', '총 발행금액', '총발행금액', '사채의 종류', '사채종류', '기초자산 종목명', '기초자산명', '발행조건 확정', '수요예측 결과', '확정금액']):
            score += 20.0
        ...
        # 법적 한도/면책/70% 미달 노이즈 문구 대폭 감점 (감점)
        if any(k in sentence for k in ['70%에 미달하는 경우', '70%에 해당하는 가액', '새로운 전환가액으로 본다', '발행 당시 전환가액']):
            score -= 15.0
